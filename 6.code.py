# -*- coding: utf-8 -*-
"""
Created on Sun Oct 23 11:49:38 2016
...
def calculate(v,theta):
    ...
    while (y[-1] >= 0):
        ...
        vx.append(vx[i - 1] - dt*0.00004*math.sqrt((vx[i - 1]+4.47)**2+vy[i - 1]**2)*(vx[i - 1]+4.47)*(1-(0.0065*y[i - 1])/280)**2.5)
        y.append(y[i - 1] + vy[i - 1]*dt)
        vy.append(vy[i - 1]-9.8*dt -dt*0.00004*math.sqrt((vx[i - 1]+4.47)**2+vy[i - 1]**2)*vy[i - 1]*(1-(0.0065*y[i - 1])/280)**2.5)
        t.append(t[i - 1] + dt)
        i+= 1
    r = y[-2] / y[-1]
    x[-1] = (x[-2] + r * x[-1]) / (r + 1)
...
#    theta0 = 180*math.atan(y_target/x_target)/math.pi
    data_record = deep_scan(100,15,x_target,y_target,5)   ### 3 for grade 1, 4 for grade 2 and 5 for grade 3
    v = data_record[0]
    theta = data_record[1]
    ...
    plt.title('Trajectory of cannon shell with air resistance,density correction and a head wind')
    plt.xlabel('x(m)')
    plt.ylabel('y(m)')
    plt.plot(x_target,y_target,'rp',linewidth = 10,label='Target')
    plt.plot(cannon_record[0],cannon_record[1],'c',label= 'Trajectroy')
