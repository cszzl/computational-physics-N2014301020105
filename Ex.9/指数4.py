# -*- coding: utf-8 -*-
"""
Created on Sun Nov 20 21:24:23 2016
...
class billiard_circle():
    ...
    def motion_calculate(self):
        ...
        for i in range(1,self.N):
            ...
            if (np.sqrt( self.x[i]**2+(self.y[i])**2 ) > 1.0) and self.y[i]>0:
            ...
            elif (np.sqrt( self.x[i]**2+(self.y[i])**2 ) > 1.0) and self.y[i]<0:
                self.x[i],self.y[i] = self.correct('np.sqrt(x**2+y**2) < 1.0',self.x[i - 1], self.y[i - 1], self.vx[i - 1], self.vy[i - 1])
                self.vx[i],self.vy[i] = self.reflect2(self.x[i],self.y[i],self.vx[i - 1], self.vy[i - 1])
            elif (self.x[i] < -1.0) and self.y[i]>0 and self.y[i]<0:
...
plt.title('Stadium with $\\alpha$=0 - divergence of two trajectories')
plt.xlabel('time')
